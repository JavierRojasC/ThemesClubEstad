...
\begin{document}  %%%%%%% main document %%%%%%%%%%%%%%%
    ...
    \begin{flushleft}
       ...
       %\normalsize{ $if(subtitle)$ $if(UP_subtitle)$ \uppercase{$subtitle$} $else$ $subtitle$ $endif$ $else$ No Subtitle $endif$ }
      ...
      % $if(author2)$ \textbf{$author2$}  \\ $endif$
      %     \vspace{-0.2cm}
      % $if(ID2)$ $ID2$ \\ $endif$
      %     \vspace{-0.2cm}
      % $if(address2)$ $address2$  \\ $endif$
      %     \vspace{-0.2cm}
      % $if(author3)$ \textbf{$author3$}  \\ $endif$
      % $if(ID3)$ $ID3$ \\ $endif$
      % $if(address3)$ $address3$  \\ $endif$

      %  \vspace{1cm}
      %
      %   $if(author4)$ \textbf{$author4$}  \\ $endif$
      % $if(ID4)$ $ID4$ \\ $endif$
      % $if(address4)$ $address4$  \\ $endif$

      %  \vspace{1cm}
      %
      % $if(author5)$ \textbf{$author5$}  \\ $endif$
      % $if(ID5)$ $ID5$ \\ $endif$
      % $if(address5)$ $address5$  \\ $endif$

      %  \vspace{1cm}
      %
      %
      % $if(author6)$ \textbf{$author6$}  \\ $endif$
      % $if(ID6)$ $ID6$ \\ $endif$
      % $if(address6)$ $address6$  \\ $endif$
